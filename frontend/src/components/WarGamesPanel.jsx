...
const WarGamesPanel = ({ savedRuns, onClearRuns, onSaveRun, canSave, isAutoRunning, onRunScenario }) => {
    ...
    return (
        <div className="flex flex-col gap-6 sm:gap-8 w-full max-w-6xl mx-auto pb-12 animate-fade-in">
            <div className="panel p-4 sm:p-6 border-l-2 border-l-purple-500/40 relative overflow-hidden">
                <div className="flex items-start gap-3 sm:gap-4 relative z-10">
                    <div className="bg-purple-500/10 p-2 sm:p-2.5 rounded-lg border border-purple-500/20 shrink-0 mt-0.5">
                        <svg className="w-5 sm:w-6 h-5 sm:h-6 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8 items-start">
                <div className="xl:col-span-2 panel p-4 sm:p-6">
                    <h3 className="text-base sm:text-lg font-display font-bold text-white mb-4 sm:mb-6 flex items-center gap-3">
                    ...
                    {loadError ? (
                        <div className="py-12 text-center text-red-400 text-sm flex flex-col items-center gap-3">
                            ...
                            <button onClick={() => window.location.reload()} className="min-h-[44px] px-4 py-2 border border-red-500/20 text-red-400 hover:bg-red-500/10 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors">
                                Retry
                            </button>
                        </div>
                    ) : scenarios.length === 0 ? (
                                ...
                                <div key={idx} className="bg-slate-800/40 rounded-lg border border-slate-700/40 transition-colors hover:bg-slate-800/60 hover:border-purple-500/20 flex flex-col h-full overflow-hidden group">
                                    <div className="p-3 sm:p-4 flex-1 flex flex-col">
                                        <div className="flex justify-between items-start mb-2 gap-2">
                                            <h4 className="font-bold text-white text-sm sm:text-[15px] leading-tight group-hover:text-purple-400 transition-colors">{s.name}</h4>
                                        ...
                                        <p className="text-slate-400 text-xs sm:text-[13px] leading-relaxed mb-3 flex-1">{s.description}</p>
                                    </div>
                                    <button 
                                        onClick={() => onRunScenario(s.request, s.gpu_state)}
                                        className="min-h-[44px] w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 text-xs sm:text-[13px] tracking-wide uppercase transition-colors flex items-center justify-center gap-2"
                                    >
                ...
                <div className="panel p-4 sm:p-6 sticky top-6">
                    <h3 className="text-base sm:text-lg font-display font-bold text-white flex items-center gap-3 mb-4 sm:mb-6">
                        <span className="bg-slate-800/80 p-2 rounded-lg border border-slate-700/40">
                            <svg className="w-5 h-5 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            {savedRuns.length > 0 && (
                <div className="panel p-4 sm:p-6 mt-4 sm:mt-8">
                    <div className="flex items-center justify-between mb-4 sm:mb-6 gap-3">
                        <h2 className="text-base sm:text-lg font-display font-bold text-white flex items-center gap-3">
                            <span className="bg-slate-800/80 p-2 rounded-lg border border-slate-700/40">
                        ...
                        <button onClick={onClearRuns} className="min-h-[40px] text-xs sm:text-sm border border-red-500/20 text-red-400 hover:text-white transition-colors flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg hover:bg-red-600">
                            Clear
                        </button>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                        {savedRuns.map((run, idx) => (
                            <div key={idx} className="bg-slate-800/50 border border-slate-700/40 rounded-lg p-4 sm:p-5 flex flex-col gap-3 sm:gap-4">
                                <h3 className="text-sm sm:text-[15px] font-bold text-white flex justify-between items-center gap-2">
                                    <span className="truncate">{run.name}</span>
                                    <span className="text-[10px] sm:text-xs bg-indigo-500/10 text-indigo-400 px-2 py-1 rounded border border-indigo-500/20 shrink-0">ROI: {run.metrics.roi_percentage.toFixed(1)}%</span>
                                </h3>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="bg-slate-900/50 rounded-lg p-2.5 sm:p-3 border border-slate-700/30">
                                        ...
                                        <p className="text-primary-400 font-mono font-bold text-sm sm:text-[15px]">${run.metrics.total_revenue.toLocaleString()}</p>
                                    </div>
                                    <div className="bg-slate-900/50 rounded-lg p-2.5 sm:p-3 border border-slate-700/30">
                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">Trust</p>
                                        <p className={`${run.metrics.trust_score >= 80 ? 'text-green-400' : run.metrics.trust_score >= 50 ? 'text-yellow-400' : 'text-red-400'} font-mono font-bold text-sm sm:text-[15px]`}>{run.metrics.trust_score.toFixed(1)}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
