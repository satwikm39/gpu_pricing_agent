    ...
    return (
        <article className={`card bg-slate-800/30 border-l-4 ${getBorderColor()} rounded-r-lg rounded-l-sm p-4 sm:p-5 flex flex-col gap-3 sm:gap-4 animate-fade-in hover:bg-slate-800/50 transition-colors`}>
            <div className="flex flex-col sm:flex-row justify-between items-start gap-2 sm:gap-3">
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
                        <span className="font-mono text-xs sm:text-sm text-slate-400 bg-slate-800 px-2 py-0.5 rounded">{request.request_id}</span>
                        <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-widest text-slate-500 truncate max-w-[160px] sm:max-w-none">{request.region} · {request.gpu_type}</span>
                    </div>
                    <div className="flex items-center gap-1.5 sm:gap-2 mt-2 flex-wrap">
                        ...
                        {state.cost_recovered ? (
                            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border bg-emerald-500/8 text-emerald-400 border-emerald-500/20 flex items-center gap-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" aria-hidden="true"></span>
                                Paid
                            </span>
                        ) : (
                            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border bg-amber-500/8 text-amber-400/80 border-amber-500/15 flex items-center gap-1">
                        ...
                        {state.market_price_per_hour && (
                            <span className="hidden sm:flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border bg-pink-500/8 text-pink-400 border-pink-500/20">
                    ...
                    <h3 className="text-lg sm:text-xl font-display font-bold text-white mt-3 flex items-center gap-2 sm:gap-3 flex-wrap">
                        {request.quantity}x <span className={`px-2 py-0.5 rounded text-[10px] sm:text-sm uppercase tracking-widest ${isSpot ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/15' : 'bg-primary-500/10 text-primary-400 border border-primary-500/15'}`}>{request.workload_type}</span>
                        <span className="text-slate-400 font-sans font-normal text-xs sm:text-sm tracking-wider uppercase">({request.duration_hours}h)</span>
                    </h3>
                </div>
                <div className="flex sm:flex-col items-center sm:items-end gap-2 shrink-0">
                    <div className={`px-3 sm:px-4 py-1 sm:py-1.5 rounded-md text-xs sm:text-sm font-display font-bold border ${getBadgeColor()} uppercase tracking-wider`}>
                        {actionBadge}
                    </div>
                </div>
            </div>
                ...
                {isOnDemand && isOverride ? (
                    <div className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                        <div className="flex flex-col">
                            <span className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-0.5">Base</span>
                            <span className="text-slate-400 font-mono text-base sm:text-lg line-through decoration-red-500/40">${basePrice.toFixed(2)}<span className="text-xs">/hr</span></span>
                        </div>
                        <div className="flex flex-col sm:items-end">
                            <span className="text-yellow-400 text-[10px] font-bold uppercase tracking-wider mb-0.5 flex items-center gap-1.5">
                            ...
                            <span className="text-yellow-400 font-mono font-bold text-lg sm:text-xl">${displayPrice.toFixed(2)}<span className="text-sm opacity-80">/hr</span></span>
                        </div>
                    </div>
                ) : (
                    <>
                        <span className={`font-display font-bold uppercase tracking-wider text-xs flex items-center gap-2 ${isApprove || isEvict ? 'text-green-400' : isOverride ? 'text-yellow-400' : isReject ? 'text-red-400' : 'text-slate-400'}`}>
                            <svg className="w-4 h-4 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                        ...
                        <div className="flex items-center gap-4 sm:gap-6">
                            {!isReject && (
                                <div className="flex flex-col items-end border-r border-slate-700/40 pr-4 sm:pr-6">
                                    <span className="text-slate-500 text-[9px] uppercase font-bold tracking-widest">Base</span>
                                    <span className="text-slate-400 font-mono text-sm">${basePrice.toFixed(2)}<span className="text-[10px]">/hr</span></span>
                                </div>
                            )}
                            <span className={`font-mono font-bold text-lg sm:text-xl ${isApprove || isEvict ? 'text-green-400' : isOverride ? 'text-yellow-400' : isReject ? 'text-red-400' : 'text-slate-400'}`}>
            ...
            <div className="bg-slate-800/40 p-4 sm:p-5 rounded-lg border border-slate-700/30 relative overflow-hidden group hover:border-accent-500/15 transition-colors">
                ...
                <div className="text-[11px] text-accent-400 font-display font-bold tracking-widest uppercase mb-2 sm:mb-3 flex items-center gap-2">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                    </svg>
                    Explanation
                </div>
                <div className="text-slate-300 text-sm leading-relaxed antialiased font-light tracking-wide border-l-2 border-slate-700/30 pl-3 sm:pl-4 py-1 sm:py-2 space-y-2 sm:space-y-3">
                ...
                {decision.target_eviction_id && (
                    <div className="mt-3 sm:mt-4 inline-flex bg-red-500/8 text-red-400 text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-md border border-red-500/20 font-mono items-center gap-2">
